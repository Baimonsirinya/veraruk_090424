...
$stmt = $pdo->prepare("SELECT * FROM users WHERE role = 'manager' AND (citizen_id LIKE :search OR name LIKE :search OR lastname LIKE :search OR Tel LIKE :search)");
...
if ($stmt->rowCount() > 0) {
    ...
    echo "<div style='text-align:center;'>ไม่พบข้อมูลผู้จัดการ</div>";
}
